...
- **Image slideshow (TikTok photo post)** → yt-dlp can't fetch the slides, so the script falls back to `gallery-dl` when it is on PATH. The report lists frames as `slide N` with no timeline: Read every slide and treat the caption (Title) plus on-slide text as the content. A yt-dlp `ERROR` on stderr above a successful report is expected here. If stderr says to install gallery-dl, tell the user (`pipx install gallery-dl` or `brew install gallery-dl`).
